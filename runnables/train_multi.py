...
def main(args: DictConfig):
    ...
    # class EarlyStopping(PL_EarlyStopping):
    #     def __init__(self, patience=10, verbose=False, delta=0):
    #         super().__init__(monitor='val_loss', patience=patience, verbose=verbose, delta=delta)

    #     def on_validation_end(self, trainer, pl_module):
    #         # Custom logic after each validation phase
    #         pass

    # Create the learning rate scheduler (ReduceLROnPlateau)
    # optimizer = torch.optim.Adam(model.parameters(), lr=args.exp.alpha_rate)
    # scheduler = ReduceLROnPlateau(optimizer, 'min', factor=0.5, patience=5, min_lr=1e-6, verbose=True)

    # Create the early stopping instance
    # Correct initialization of EarlyStopping callback
    # early_stopping = EarlyStopping(
    #     monitor='val_loss',  # metric to monitor
    #     patience=20,  # number of epochs with no improvement after which training will stop
    #     verbose=True,  # whether to print messages when stopping
    ...
    if args.exp.logging:
        ...
        artifacts_path = "/Users/sneha/Downloads/CausalTransformer-main/config/"
    else:
        mlf_logger = None
        artifacts_path = None
    ...
    multimodel_trainer = Trainer(logger=mlf_logger, max_epochs=args.exp.max_epochs,
